...
* [Why do we need this ESP32_W5500_Manager library](#why-do-we-need-this-ESP32_W5500_Manager-library)
  * [Features](#features)
...
* [Debug Terminal Output Samples](#debug-terminal-output-samples)
  * [1. ConfigOnDoubleReset_TZ using LittleFS on ESP32_DEV with ESP32_W5500](#1-ConfigOnDoubleReset_TZ-using-LittleFS-on-ESP32_DEV-with-ESP32_W5500)
...
### Why do we need this [ESP32_W5500_Manager library](https://github.com/khoih-prog/ESP32_W5500_Manager)
...
This [**ESP32_W5500_Manager** library](https://github.com/khoih-prog/ESP32_W5500_Manager) currently supports these following boards:
...
The best and easiest way is to use `Arduino Library Manager`. Search for `ESP32_W5500_Manager`, then select / install the latest version. You can also use this link [![arduino-library-badge](https://www.ardu-badge.com/badge/ESP32_W5500_Manager.svg?)](https://www.ardu-badge.com/ESP32_W5500_Manager) for more detailed instructions.

### Manual Install

1. Navigate to [ESP32_W5500_Manager](https://github.com/khoih-prog/ESP32_W5500_Manager) page.
2. Download the latest release `ESP32_W5500_Manager-main.zip`.
3. Extract the zip file to `ESP32_W5500_Manager-main` directory 
4. Copy the whole `ESP32_W5500_Manager-main` folder to Arduino libraries' directory such as `~/Arduino/libraries/`.

### VS Code & PlatformIO:

1. Install [VS Code](https://code.visualstudio.com/)
2. Install [PlatformIO](https://platformio.org/platformio-ide)
3. Install [**ESP32_W5500_Manager** library](https://registry.platformio.org/libraries/khoih-prog/ESP32_W5500_Manager) by using [Library Manager](https://registry.platformio.org/libraries/khoih-prog/ESP32_W5500_Manager/installation). Search for **ESP32_W5500_Manager** in [Platform.io Author's Libraries](https://platformio.org/lib/search?query=author:%22Khoi%20Hoang%22)
...
Thanks to [Roshan](https://github.com/solroshan) to report the issue in [Error esp_littlefs.c 'utime_p'](https://github.com/khoih-prog/ESP32_W5500_Manager/issues/28) 
...
```cpp
#include <ESP32_W5500_Manager.hpp>               //https://github.com/khoih-prog/ESP32_W5500_Manager
```

in many files. But be sure to use the following `#include <ESP32_W5500_Manager.h>` **in just 1 `.h`, `.cpp` or `.ino` file**, which must **not be included in any other file**, to avoid `Multiple Definitions` Linker Error
...
Have a look at the discussion in [Different behaviour using the src_cpp or src_h lib #80](https://github.com/khoih-prog/ESP32_W5500_Manager/discussions/80)
...
```cpp
//ESP32_W5500_manager.setSTAStaticIPConfig(stationIP, gatewayIP, netMask, dns1IP, dns2IP);
ESP32_W5500_manager.setSTAStaticIPConfig(WM_STA_IPconfig);
```
...
```cpp
const char * TZ_Result = ESP32_W5500_manager.getTZ(_timezoneName);
```
...
```cpp
ESP32_W5500_Manager ESP32_W5500_manager("Personalized-HostName");
```

then later call

```cpp
ESP32_W5500_manager.startConfigPortal()
```
...
These illustrating steps is based on the example [ConfigOnSwitchFS](https://github.com/khoih-prog/ESP32_W5500_Manager/tree/main/examples/ConfigOnSwitchFS)
...
The example [ConfigOnSwitchFS](https://github.com/khoih-prog/ESP32_W5500_Manager/tree/main/examples/ConfigOnSwitchFS) will open the CP whenever a SW press is detected in `loop()`. So the code to add `dynamic variables` will be there, just after the CP `ESP32_W5500_Manager` class initialization to create `ESP32_W5500_Manager` object.
...
// is configuration portal requested?
  if ((digitalRead(TRIGGER_PIN) == LOW) || (digitalRead(TRIGGER_PIN2) == LOW))
  {
    ...
    ESP32_W5500_Manager ESP32_W5500_manager("ConfigOnSwitchFS");
...
Adding those `ESP32_EMParameter` objects created in Step 2 using the function `addParameter()` of object `ESP32_W5500_Manager`
...
ESP32_W5500_manager.addParameter(&p_thingspeakApiKey);
ESP32_W5500_manager.addParameter(&p_sensorDht22);
ESP32_W5500_manager.addParameter(&p_pinSda);
ESP32_W5500_manager.addParameter(&p_pinScl);
```
...
After users select `Save`, the CP `ESP32_W5500_Manager` object will save the user input data into related `ESP32_EMParameter` objects.
...
<p align="center">
    <img src="https://github.com/khoih-prog/ESP32_W5500_Manager/raw/main/Images/Main.png">
</p>

Select `Information` to enter the Info page where the board info will be shown (long page)

<p align="center">
    <img src="https://github.com/khoih-prog/ESP32_W5500_Manager/raw/main/Images/Info.png">
</p>
...
<p align="center">
    <img src="https://github.com/khoih-prog/ESP32_W5500_Manager/raw/main/Images/Configuration_Standard.png">
</p>
...
<p align="center">
    <img src="https://github.com/khoih-prog/ESP32_W5500_Manager/raw/main/Images/Saved.png">
</p>
...
```cpp
ESP32_W5500_manager.setSaveConfigCallback(saveConfigCallback);
```
...
```cpp
ESP32_W5500_manager.setConfigPortalTimeout(120);
```
...
void loop()
{
  ...
  if ((digitalRead(TRIGGER_PIN) == LOW) || (digitalRead(TRIGGER_PIN2) == LOW))
  {
    ...
    ESP32_W5500_Manager ESP32_W5500_manager("ConfigOnSwitch");
    ...
    ESP32_W5500_manager.setSTAStaticIPConfig(stationIP, gatewayIP, netMask);
#endif
#endif
    ...
    if (loadConfigData())
    {
      ESP32_W5500_manager.setConfigPortalTimeout(
    ...
    else
    {
      ...
      ESP32_W5500_manager.setConfigPortalTimeout(0);
...
Many applications need configuration parameters like `MQTT host and port`, [Blynk](http://www.blynk.cc) or [emoncms](http://emoncms.org) tokens, etc. While it is possible to use [`ESP32_W5500_Manager`](https://github.com/khoih-prog/ESP32_W5500_Manager) to collect additional parameters, it is better to read these parameters from a web service once [`ESP32_W5500_Manager`](https://github.com/khoih-prog/ESP32_W5500_Manager) has been used to connect to the Internet.

To capture other parameters with [`ESP32_W5500_Manager`](https://github.com/khoih-prog/ESP32_W5500_Manager) is a little bit more complicated than all the other features. This requires adding custom HTML to your form. 

If you want to do it with [`ESP32_W5500_Manager`](https://github.com/khoih-prog/ESP32_W5500_Manager) see the example [ConfigOnSwitchFS](examples/ConfigOnSwitchFS)
...
```cpp
ESP32_W5500_manager.setSTAStaticIPConfig(IPAddress(192,168,2,232), IPAddress(192,168,2,1), IPAddress(255,255,255,0));
```
...
```cpp
ESP32_W5500_manager.setCustomHeadElement("<style>html{filter: invert(100%); -webkit-filter: invert(100%);}</style>");
```
...
ESP32_W5500_manager.addParameter(&custom_text);
```
...
https://github.com/khoih-prog/ESP32_W5500_Manager/blob/3af21a7a32dc5fdc7e9712aefee506ded3733479/examples/ConfigOnSwitch/ConfigOnSwitch.ino#L27-L916
...
#### 1. ConfigOnDoubleReset_TZ using LittleFS on ESP32_DEV with ESP32_W5500

##### 1.1 DRD => Config Portal

This is terminal debug output when running [ConfigOnDoubleReset](examples/ConfigOnDoubleReset) on  **ESP32_W5500**. `Config Portal` was requested by DRD to input and save Credentials, such as Static IP address.

```
Starting ConfigOnDoubleReset_TZ using LittleFS on ESP32_DEV with ESP32_W5500
ESP32_W5500_Manager v1.0.0
...
Saving config file...
Saving config file OK
Local Date/Time: Sat Dec 10 22:04:59 2022
Local Date/Time: Sat Dec 10 22:05:59 2022
Local Date/Time: Sat Dec 10 22:06:59 2022
Local Date/Time: Sat Dec 10 22:07:59 2022
```
...
Config file was successfully saved
[EM] freeing allocated params!
Local Date/Time: Sat Dec 10 22:08:59 2022
Local Date/Time: Sat Dec 10 22:09:59 2022
Local Date/Time: Sat Dec 10 22:10:59 2022
Local Date/Time: Sat Dec 10 22:11:59 2022
```
...
```cpp
ESP32_W5500_manager.setDebugOutput(false);
```
...
Submit issues to: [ESP32_W5500_Manager issues](https://github.com/khoih-prog/ESP32_W5500_Manager/issues)
...
- The library is licensed under [MIT](https://github.com/khoih-prog/ESP32_W5500_Manager/blob/main/LICENSE)
